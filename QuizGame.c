...
javagame:
     ...
     switch(r)
		...
		printf("\n\nA.Dynamic\t\tB.Architecture Neutral\n\nC.Use of pointers\t\tD.Object riented");
		if (toupper(getch())=='C')
			{printf("\n\nCorrect!!!");countr++;getch(); break;}
		else
		       {printf("\n\nWrong!!! The correct answer is C.Use of pointers");getch();
		       goto score;
		       break;}

        case 10:
		printf("\n\n\nIn which process, a local variable has the same name as one of the instance variables?");
		printf("\n\nA.Serialization\t\tB.Variable Shadowing\n\nC.Abstraction\t\tD.Multi-threading");
		if (toupper(getch())=='B')
			{printf("\n\nCorrect!!!");countr++;getch(); goto score; break;}
		else
		       {printf("\n\nWrong!!! The correct answer is B.Variable shadowing");getch();
		       goto score;
		       break;}
        }}
	score:
